    ...
    <div className="grid gap-3">
      {steps.map((step, index) => (
        <div key={step} className="flex items-center gap-3">
          <span className={`grid h-7 w-7 place-items-center border text-xs font-bold ${index <= current ? "border-accent bg-accent text-white" : "border-line bg-white text-muted"}`}>{index + 1}</span>
