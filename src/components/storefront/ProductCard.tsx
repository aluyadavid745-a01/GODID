...
export const ProductCard = ({ product }: { product: Product }) => {
  ...
  const [wishlisted, setWishlisted] = useState(() => {
    try { return (JSON.parse((typeof window !== "undefined" ? localStorage.getItem("godid-wishlist") : null) ?? "[]") as string[]).includes(product.id); }
    catch { return false; }
  });
  const firstAvailable = product.variants.find((variant) => variant.inventory > 0);
  const toggleWishlist = () => {
    let saved: string[] = [];
    try { saved = JSON.parse((typeof window !== "undefined" ? localStorage.getItem("godid-wishlist") : null) ?? "[]") as string[]; }
  ...
  return (
    <article className="group relative min-w-0">
      <Link href={`/product/${product.slug}`} className="block overflow-hidden bg-line">
      ...
      <div className="mt-3 grid gap-3 sm:mt-4 sm:flex sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <Link href={`/product/${product.slug}`} className="block text-sm font-semibold leading-snug hover:text-accent sm:text-base">{product.name}</Link>
